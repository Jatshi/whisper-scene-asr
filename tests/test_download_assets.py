...
def test_fetch_aishell_extracts_nested_speaker_archives(tmp_path) -> None:
    ...
    dataset = source / "data_aishell"
    (dataset / "transcript").mkdir(parents=True)
    (dataset / "transcript" / "aishell_transcript_v0.8.txt").write_text("UTT text\n", encoding="utf-8")
    inner_content = tmp_path / "inner" / "S0001"
    inner_content.mkdir(parents=True)
    (inner_content / "UTT.wav").write_bytes(b"wav")
    (dataset / "wav").mkdir()
