...
def test_build_manifests_and_quality_report(tmp_path) -> None:
    root = tmp_path / "data_aishell"; transcript = root / "transcript" / "aishell_transcript_v0.8.txt"
    transcript.parent.mkdir(parents=True); transcript.write_text("UTT1 你 好！\nUTT2 测 试\nUTT3 开 发\n", encoding="utf-8")
    for split, utt in (("train", "UTT1"), ("dev", "UTT2"), ("test", "UTT3")):
        _write_wave(root / "wav" / split / "S0001" / f"{utt}.wav")
    output = tmp_path / "processed"; assert build_manifests(root, output) == {"train": 1, "validation": 1, "test": 1}
